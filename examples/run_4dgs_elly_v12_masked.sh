#!/usr/bin/env bash
# Run 4DGS on elly — 150 frames, v12 config + masks
# v12: based on v10 but with opacity/SH deform DISABLED (fix for opacity collapse)
#   - MLP depth 2, time_pe_bands 8 (higher capacity)
#   - opacity_reg 0.01 to keep opacities healthy
#   - NO enable_opacity_deform (was causing death spiral with density control)
#   - NO enable_sh_deform (adds instability, test separately)
set -e
...
RESULT_DIR=/data/shared/elaheh/4D_demo/elly_4dgs_f150_v12_masked
