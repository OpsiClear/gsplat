...
class Config:
    ...
    # Per-Gaussian dynamic perception coefficient w (stored as logit in splats["w"]).
    # sigmoid(w) → 1 = fully dynamic (full deformation), → 0 = fully static (no deformation).
    sdd_mode: bool = True
    # Learning rate for w
    w_lr: float = 1e-3
    # Rate for progressive binary entropy schedule λ_bi(step) = max_w * (1 - exp(-alpha*step))
    binary_entropy_alpha: float = 1e-4
    # Max weight for L_bi (binary entropy loss that pushes w toward 0 or 1)
    binary_entropy_weight: float = 0.01
    # Weight for L_asg (automatic supervision guidance: weighted static/dynamic renders)
    lasg_weight: float = 1.0
    # Threshold tau during training: w < tau → static render, w >= tau → dynamic render
    train_threshold: float = 0.5
    # Tightened threshold for dynamic Gaussians at inference/PLY export
    infer_dynamic_threshold: float = 0.85
    # Tightened threshold for static Gaussians at inference/PLY export
    infer_static_threshold: float = 0.2
...
def create_splats_with_optimizers(
    ...
    if init_type == "ply":
        ...
        if include_w:
            params.append(("w", torch.nn.Parameter(torch.zeros(N, device=device)), w_lr))
    else:
        if init_type == "sfm":
...
class Runner:
    ...
    def __init__(self, local_rank: int, world_rank: int, world_size: int, cfg: Config):
        ...
        self.splats, self.optimizers = create_splats_with_optimizers(
            ...
            include_w=cfg.sdd_mode,
            w_lr=cfg.w_lr,
        )
        print(f"[4DGS] Canonical Gaussians: {len(self.splats['means'])}")
        if cfg.sdd_mode:
            print(f"[SDD] w init: sigmoid(w).mean()={torch.sigmoid(self.splats['w']).mean():.4f} (expected ~0.5)")
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        if deformation_active:
            ...
            self._last_deltas = deltas  # cache for L_asg (avoids 2nd deformation forward pass)
            w_gate = torch.sigmoid(self.splats["w"]) if cfg.sdd_mode and "w" in self.splats else None
            means, quats, scales, opacities, colors = apply_deformation(
                self.splats, deltas, w=w_gate
            )
        else:
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            # ---- SDD-4DGS Losses (fine phase only) --------------------------------
            lbi = torch.tensor(0.0, device=device)
            lambda_bi = 0.0
            lasg_loss = torch.tensor(0.0, device=device)
            if cfg.sdd_mode and deform_active and "w" in self.splats:
                w = torch.sigmoid(self.splats["w"])  # [N]

                # L_bi: binary entropy with progressive schedule (paper eq. 9, 10)
                if cfg.binary_entropy_weight > 0:
                    lambda_bi = (1.0 - math.exp(-cfg.binary_entropy_alpha * step)) * cfg.binary_entropy_weight
                    w_clamp = w.clamp(1e-6, 1 - 1e-6)
                    lbi = -(w_clamp * torch.log(w_clamp) + (1 - w_clamp) * torch.log(1 - w_clamp)).mean()
                    loss = loss + lambda_bi * lbi

                # L_asg: automatic supervision guidance (paper eq. 11)
                if cfg.lasg_weight > 0 and hasattr(self, '_last_deltas'):
                    # Recompute deformed params with w-gated deltas (cheap: reuses cached deltas)
                    means_w, quats_w, scales_w, opacs_w, colors_w = apply_deformation(
                        self.splats, self._last_deltas, w=w
                    )
                    viewmats = torch.linalg.inv(camtoworlds)
                    rast_kw = dict(
                        ...
                        rasterize_mode="antialiased" if cfg.antialiased else "classic",
                        camera_model=cfg.camera_model,
                        with_ut=cfg.with_ut, with_eval3d=cfg.with_eval3d,
                    )
                    tau = cfg.train_threshold

                    # Motion mask: render sigmoid(w) as pixel-space feature (no gradient needed)
                    # sh_degree=None → raw [N, D] RGB mode (not SH)
                    w_color = w.detach().unsqueeze(-1).expand(-1, 3)  # [N, 3]
                    with torch.no_grad():
                        m_render, _, _ = rasterization(
                            means=means_w.detach(), quats=quats_w.detach(),
                            scales=scales_w.detach(), opacities=opacs_w.detach(),
                            colors=w_color, sh_degree=None, **rast_kw
                        )
                    m = m_render[..., :1]  # [B, H, W, 1] — motion probability map (detached)

                    static_mask = (w < tau).detach()   # [N] bool
                    dyn_mask = (~static_mask)

                    if static_mask.any() and dyn_mask.any():
                        # Static render: canonical (non-deformed) params for w < tau
                        Is, _, _ = rasterization(
                            means=self.splats["means"][static_mask],
                            quats=F.normalize(self.splats["quats"][static_mask], p=2, dim=-1),
                            scales=torch.exp(self.splats["scales"][static_mask]),
                            opacities=torch.sigmoid(self.splats["opacities"][static_mask]),
                            colors=torch.cat([self.splats["sh0"][static_mask],
                                              self.splats["shN"][static_mask]], dim=1),
                            sh_degree=sh_degree_to_use, **rast_kw
                        )
                        # Dynamic render: w-gated deformed params for w >= tau
                        Id, _, _ = rasterization(
                            means=means_w[dyn_mask], quats=quats_w[dyn_mask],
                            scales=scales_w[dyn_mask], opacities=opacs_w[dyn_mask],
                            colors=colors_w[dyn_mask], sh_degree=sh_degree_to_use, **rast_kw
                        )
                        # L_asg = (1-m)*L1(Is, target) + m*L1(Id, target) (paper eq. 11)
                        l1_s = F.l1_loss(Is[..., :3], pixels, reduction='none')
                        l1_d = F.l1_loss(Id[..., :3], pixels, reduction='none')
                        lasg_loss = ((1 - m) * l1_s + m * l1_d).mean()
                        loss = loss + cfg.lasg_weight * lasg_loss
            ...
            if world_rank == 0 and cfg.tb_every > 0 and step % cfg.tb_every == 0:
                ...
                if cfg.sdd_mode and deform_active and "w" in self.splats:
                    self.writer.add_scalar("train/lbi", lbi.item(), step)
                    self.writer.add_scalar("train/lambda_bi", lambda_bi, step)
                    self.writer.add_scalar("train/lasg", lasg_loss.item(), step)
                    self.writer.add_scalar("train/w_mean", torch.sigmoid(self.splats["w"]).mean().item(), step)
                ...
                if cfg.tb_image_every > 0 and step % cfg.tb_image_every == 0:
                    with torch.no_grad():
                        ...
                        if cfg.sdd_mode and "w" in self.splats:
                            self.writer.add_histogram(
                                "train/w_sigmoid",
                                torch.sigmoid(self.splats["w"]).detach().cpu(), step
                            )
    ...
    def export_per_frame_plys(self):
        ...
        for frame_rank in range(num_frames):
            ...
            # SDD: color-coded + static/dynamic split PLYs
            if cfg.sdd_mode and "w" in self.splats:
                w = torch.sigmoid(self.splats["w"])

                # Color-coded debug PLY: red=dynamic (w→1), blue=static (w→0)
                rgb_debug = torch.stack([w, torch.zeros_like(w), 1.0 - w], dim=-1)  # [N,3]
                sh0_debug = rgb_to_sh(rgb_debug).unsqueeze(1)  # [N,1,3]
                shN_debug = torch.zeros_like(self.splats["shN"])
                export_splats(
                    means=means_d, scales=scales_log, quats=quats_d,
                    opacities=opacs_logit, sh0=sh0_debug, shN=shN_debug,
                    format="ply",
                    save_to=f"{out_dir}/frame_{frame_idx:06d}_wcolor.ply",
                )

                # Static-only PLY: w < infer_static_threshold, canonical params (no deform)
                static_mask = (w < cfg.infer_static_threshold)
                if static_mask.any():
                    export_splats(
                        means=self.splats["means"][static_mask],
                        scales=self.splats["scales"][static_mask],
                        quats=self.splats["quats"][static_mask],
                        opacities=self.splats["opacities"][static_mask],
                        sh0=self.splats["sh0"][static_mask],
                        shN=self.splats["shN"][static_mask],
                        format="ply",
                        save_to=f"{out_dir}/frame_{frame_idx:06d}_static.ply",
                    )

                # Dynamic-only PLY: w > infer_dynamic_threshold, with deformation baked in
                dyn_mask = (w > cfg.infer_dynamic_threshold)
                if dyn_mask.any():
                    export_splats(
                        means=means_d[dyn_mask],
                        scales=torch.log(scales_d[dyn_mask]),
                        quats=quats_d[dyn_mask],
                        opacities=torch.logit(opacs_d[dyn_mask].clamp(1e-6, 1 - 1e-6)),
                        sh0=colors_d[dyn_mask, :1, :],
                        shN=colors_d[dyn_mask, 1:, :],
                        format="ply",
                        save_to=f"{out_dir}/frame_{frame_idx:06d}_dynamic.ply",
                    )
