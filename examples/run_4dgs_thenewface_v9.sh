#!/usr/bin/env bash
# Run 4DGS on thenewface — 300 frames (stride 2 from 678), v9 config
# v9: v8 base + fine detail improvements:
#   - ssim_lambda=0.2 (texture preservation)
#   - grid_resolution=64 (same as v8, 128 was unstable with multires [1,2,4,8])
#   - refine_stop_iter=30000 (more Gaussians in dynamic areas)
#   - scale_reg=0.01 (prevents oversized Gaussians)
#   - max_steps=80000 (more convergence time)
set -e

DATA_DIR=/data/shared/elaheh/4D_demo/new_data/thenewface/undistorted
RESULT_DIR=/data/shared/elaheh/4D_demo/thenewface_4dgs_f300_v9
