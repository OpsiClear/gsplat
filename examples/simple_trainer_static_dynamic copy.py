...
class Runner:
    ...
    def __init__(self, local_rank: int, world_rank: int, world_size: int, cfg: Config):
        ...
        if cfg.static_ply_path is not None:
            ...
            # Match SH bands to cfg.sh_degree (pad or trim shN)
            target_shN = (cfg.sh_degree + 1) ** 2 - 1
            loaded_shN = s_shN.shape[1]
            if loaded_shN >= target_shN:
                s_shN = s_shN[:, :target_shN, :]
            else:
                pad = torch.zeros(s_shN.shape[0], target_shN - loaded_shN, 3, device=self.device)
            ...
            self.static_activated = {
                ...
                "colors":    torch.cat([s_sh0, s_shN], dim=1), # [N_s, K, 3]
            }
            print(f"[4DGS] Static Gaussians loaded: {s_means.shape[0]} (frozen, always rendered)")
